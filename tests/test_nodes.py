...
def remove_all():
    ...
    completed_process = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--simple-list"],
    ...
    for entry in result_list:
        ...
        name = entry.split('|')[0]
        print("Removing: ", name)
        subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--remove", name], check=True)
...
def expect_empty_verbose_list():
    ...
    completed_process = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--list"], check=True, stdout=subprocess.PIPE)
...
def test_nodes():
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_ALPHA], check=True)
    validate_node_state(NODE_ALPHA, True, True)
    # Stop the node and test its state.
    #   Tests `--stop`
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--stop", NODE_ALPHA], check=True)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_BRAVO], check=True)
    ...
    #   Tests `--get-active`.
    assert subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--get-active"], check=True, \
                          stdout=subprocess.PIPE).stdout.decode().replace('\r', '') == (NODE_BRAVO + "\n")
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--set-active", NODE_ALPHA], check=True)
    validate_node_list([[NODE_ALPHA, True, False],[NODE_BRAVO, False, True]]) # Note this is TF,FT
    assert subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--get-active"], check=True, \
                          stdout=subprocess.PIPE).stdout.decode().replace('\r', '') == (NODE_ALPHA + "\n")

    # Remove NODE_ALPHA, ensure it is no longer in the list.
    #   Tests `--remove`.
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--remove", NODE_ALPHA], check=True)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_ALPHA, "--config", CONFIG_PATH], check=True)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_BRAVO, "--config", CONFIG_FILE], check=True)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--stop", NODE_BRAVO], check=True)
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_BRAVO, "--config", CONFIG_FILE_CUST], check=True)
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
    ...
    completed_process = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_ALPHA, "--config", "unknown_config"], \
                                       check=False)
    ...
    # Test `--config` alone.
    #   pylint: disable=subprocess-run-check
    completed_process = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--config", "unknown_config"], check=False)
    assert completed_process.returncode != 0
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
                        [NODE_BRAVO, True, True, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR]])
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_CHARLIE], check=True)
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--export-node", NODE_ALPHA, EXPORT_DIR], check=True)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--export-node", NODE_BRAVO, os.path.join(EXPORT_DIR, "bravo_export.tgz")], \
                   check=True)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--export-node", NODE_CHARLIE, os.path.join(EXPORT_DIR, \
                    *["new_path","charlie_export.tgz"])], check=True)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--import-node", os.path.join(EXPORT_DIR, "ALPHA_NODE.tgz"), NODE_ALPHA], \
                   check=True)
    validate_node_list([[NODE_ALPHA, True, True, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR]])

    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--import-node", os.path.join(EXPORT_DIR, "bravo_export.tgz"), NODE_BRAVO], \
                   check=True)
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
                        [NODE_BRAVO, True, True, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR]])

    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--import-node", os.path.join(EXPORT_DIR, *["new_path","charlie_export.tgz"]), \
                    NODE_CHARLIE], check=True)
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
                        [NODE_BRAVO, False, True, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
                        [NODE_CHARLIE, True, True, DEFAULT_HTTP_ADDR, DEFAULT_P2P_ADDR, DEFAULT_SHIP_ADDR]])
...
def test_start_active_node():
    ...
    #   Tests `--get-active`.
    assert subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--get-active"], check=True, \
                          stdout=subprocess.PIPE).stdout.decode().replace('\r', '') == (NODE_CHARLIE + "\n")
...
def test_node_start_rmdirtydb():
    ...
    # Create NODE_ALPHA
    res = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_ALPHA, "--rmdirtydb"], stdout=subprocess.PIPE, check=True)
    ...
    res = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_ALPHA], stdout=subprocess.PIPE, check=False)
    assert b'database dirty flag set' in res.stdout
    validate_node_state(NODE_ALPHA, True, False)

    # start NODE_ALPHA with --rmdirtydb to verify the node starts successfully
    res = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_ALPHA, "--rmdirtydb"], stdout=subprocess.PIPE, check=False)
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_BRAVO, "--rmdirtydb"], check=True)
    validate_node_state(NODE_BRAVO, True, True)
    ...
    res = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_BRAVO], stdout=subprocess.PIPE, check=False)
    assert b'database dirty flag set' in res.stdout
    validate_node_state(NODE_BRAVO, True, False)

    # start NODE_BRAVO with --rmdirtydb to verify the node starts successfully
    res = subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_BRAVO, "--rmdirtydb"], stdout=subprocess.PIPE, check=False)
    assert res.returncode == 0
    ...
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_ALPHA, "--config", CONFIG_PATH, "--rmdirtydb"], check=True)
    validate_node_list([[NODE_ALPHA, True, True, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR]])

    # use standard config.ini
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME,"--start", NODE_BRAVO, "--config", CONFIG_FILE, "--rmdirtydb"], check=True)
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
                        [NODE_BRAVO, True, True, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR]])
    ...
    # use customized .ini
    subprocess.run([DUNES_EXE, '-C', TEST_CONTAINER_NAME, "--start", NODE_BRAVO, "--config", CONFIG_FILE_CUST, "--rmdirtydb"], check=True)
    validate_node_list([[NODE_ALPHA, False, False, ALT_HTTP_ADDR, ALT_P2P_ADDR, ALT_SHIP_ADDR],
                        [NODE_BRAVO, True, True, ALT_HTTP_ADDR_CUST, ALT_P2P_ADDR_CUST, ALT_SHIP_ADDR_CUST]])
